...
//                return;
            ...
            PersonInUniversity student = studentFactory.createPerson(name,surname,id,password,email,phone,course,gender);

            DataBaseConnection.getInstance().addStudent((Student) student);

            showAlert("Success", "Student added successfully", Alert.AlertType.INFORMATION);
            addStudentButton.getScene().getWindow().hide();
        } catch (NumberFormatException e) {
            showAlert("Error", "Invalid input. Please enter valid numeric values for ID and Course.", Alert.AlertType.ERROR);
        } catch (SQLException e) {
            showAlert("Error", "Failed to add student to the database.", Alert.AlertType.ERROR);
